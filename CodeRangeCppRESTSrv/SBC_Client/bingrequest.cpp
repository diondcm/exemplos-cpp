...
*
* I Dion, edited it to make it get from our C++ rest server
*
* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
****/
...
#endif
{
    ...
    file_buffer<uint8_t>::open(outputFileName, std::ios::out).then([=](streambuf<uint8_t> outFile) -> pplx::task<http_response>
    {
        *fileBuffer = outFile; 
        ...
        http_client client(U("http://localhost:8080/datasnap/rest/TServerREST/GetCountryList"), client_config_for_proxy()); 
